...
export const getUserNotificationToken = async (userId: string) => {
  const supabase = await createClient()
    ...
    if(error){
        throw new Error(`Error fetching user notification token: ${error.message}`)
    }
    return data
}
...
export const sendNotification = async (userId: string, status: string) => {
    ...
    await sendPushNotification({expoPushToken: tokenData.expo_notification_token, title: 'Order Update', body: `Your order has been ${status}`})
